...
class GaussianMixtureModel:
    ...
    def _compute_log_likelihood(self, X):
        """
        Vectorized computation of log-likelihood for Gaussian Mixture Model.
        """
        n_samples, n_features = X.shape
        K = self.n_components

        # Preallocate: log_prob[i, k] = log(π_k * N(x_i | μ_k, Σ_k))
        log_prob = np.zeros((n_samples, K))

        for k in range(K):
            # log π_k + log N(x | μ_k, Σ_k)
            log_prob[:, k] = np.log(self.pi_[k] + 1e-12) + multivariate_normal.logpdf(
                   X, mean=self.mu_[k], cov=self.cov_[k], allow_singular=True)

         # log-likelihood for each point: log(sum_k π_k * N(x_i | μ_k, Σ_k))
         # Use log-sum-exp trick for numerical stability
        max_log = np.max(log_prob, axis=1, keepdims=True)
        log_likelihood = np.sum(max_log + np.log(np.sum(np.exp(log_prob - max_log), axis=1, keepdims=True)))
